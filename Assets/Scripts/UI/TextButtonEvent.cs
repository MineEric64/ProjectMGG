...
[Obsolete()]
public class TextButtonEvent : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
{
    public Color hover = new Color(0.00f, 0.00f, 0.00f, 0.95f);
    ...
    {
        pressed = exit;
        buttonText = this.gameObject.GetComponent<TextMeshProUGUI>();
    }
    ...
    void Update()
    {
        if (needToUpdate)
        {
            updateStart += Time.deltaTime * updateTime;
            buttonText.color = Color.Lerp(buttonText.color, desiredColor, updateStart);

            if (haveToTextGlow)
            {
                var glowColor = Color.Lerp(currentTextGlowColor, desiredTextGlowColor, updateStart);
                buttonText.fontSharedMaterial.SetColor(ShaderUtilities.ID_GlowColor, glowColor);
            }

            if (buttonText.color == desiredColor)
            {
                updateStart = 0f;
                needToUpdate = false;
            }
