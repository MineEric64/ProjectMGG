...
namespace ProjectMGG.Ingame.Script.Keywords.Renpy.ATL
{
    public class RpyAlign : IATL
    {
        ...
        public RpyAlign(float value, bool isX)
        {
        ...
        public void Interpret()
        {
            if (Texture == null)
            {
                ExceptionManager.Throw("Failed to interpret 'align' attribute in transform. The texture is null.", "Script/RpyAlign", Line);
                return;
            }
            if (Value != null && Value.Interpret() is float value)
            {
                ...
                if (IsX)
                {
                    ...
                    float offset = (value - 0.5f) * TextureSizeScaled.Width;
                    if (!isFloat) offset = value - (TextureSize.Width / 2);
                ...
                else
                {
                    ...
                    float offset = (value - 0.5f) * TextureSizeScaled.Height;
                    if (!isFloat) offset = value - (TextureSize.Height / 2);
            ...
            else ExceptionManager.Throw("Failed to interpret 'align' attribute in transform. The value is null or not a number.", "Script/RpyAlign", Line);
        }
    }
}